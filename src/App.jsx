...
const T = {
  en: {
    ...
    disclaimer_text: "NutriCrew plans are generated by AI and are for informational purposes only — not medical, dietetic, or nutritional advice. We are not licensed nutritionists or dietitians, and we do not guarantee this plan is free of any specific ingredient or allergen. Always check every ingredient and label yourself before eating, and consult a qualified healthcare professional before making significant changes to your diet or managing a medical condition.",
  ...
  fr: {
    ...
    disclaimer_text: "Les plans NutriCrew sont générés par IA et sont fournis à titre informatif uniquement — ce n'est pas un avis médical, diététique ou nutritionnel. Nous ne sommes pas des nutritionnistes ou diététiciens agréés, et nous ne garantissons pas que ce plan soit exempt d'un ingrédient ou allergène spécifique. Vérifiez toujours vous-même chaque ingrédient et étiquette, et consultez un professionnel de santé qualifié avant d'apporter des changements importants à votre alimentation ou de gérer une condition médicale.",
  ...
  es: {
    ...
    disclaimer_text: "Los planes de NutriCrew son generados por IA y son solo para fines informativos — no es un consejo médico, dietético ni nutricional. No somos nutricionistas ni dietistas certificados, y no garantizamos que este plan esté libre de algún ingrediente o alérgeno específico. Verifica siempre tú mismo cada ingrediente y etiqueta, y consulta a un profesional de la salud calificado antes de hacer cambios importantes en tu dieta o de gestionar una condición médica.",
...
function PlanScreen({ t, plan, loading, pairing, user, activeTab, setActiveTab, activeDay, setActiveDay, onNewPairing, onRetry, favorites, onToggleFavorite, onOpenAirplaneMeal, onUpdateMeal, isOnline, planKey, onShare, shareCopied, onOpenReferral, lang }) {
  ...
  return (
    <div style={styles.planScreen}>
      ...
      {/* Allergy-specific warning — only when an allergy/intolerance is active */}
      {(pairing?.diets || user?.diets || []).some(d => ALLERGY_DIET_VALUES.includes(d)) && (
        <div style={styles.allergyWarningBanner}>
          <span style={{fontSize:18}}>🚨</span>
          <div>
            <div style={styles.allergyWarningTitle}>{t.allergy_warning_title}</div>
            <div style={styles.allergyWarningText}>{t.allergy_warning_text}</div>
          </div>
        </div>
      )}
      ...
      <div style={styles.tabBar}>
        {["plan","grocery","restrictions",...(plan.performanceAdvisory ? ["performance"] : [])].map(tab => (
          <button key={tab}
            style={{...styles.tab, ...(activeTab===tab?styles.tabActive:{})}}
            onClick={() => setActiveTab(tab)}>
            {tab === "plan" ? "🍽️" : tab === "grocery" ? "🛒" : tab === "restrictions" ? "🚫" : "⚡"}
            <span style={styles.tabLabel}>
              {tab === "plan" ? t.tab_plan : tab === "grocery" ? t.tab_grocery : tab === "restrictions" ? t.tab_restrictions : t.tab_performance}
            </span>
          </button>
        ))}
      </div>
    ...
    return (
      <div style={styles.premiumScreen}>
        ...
        <div style={styles.premiumMsg}>
          {ctaBack
            ? "Your account is now active. Gym Plans, Roster Automation, Calorie Deficit plans, Jetlag Meal Plans, and Cognitive Performance meal timing are all unlocked."
...
const styles = {
  ...
  allergyWarningBanner: {
    display: "flex", gap: 12, alignItems: "flex-start",
    background: `${C.red}14`, borderLeft: `3px solid ${C.red}`,
    padding: "14px 20px", margin: "0",
  },
  allergyWarningTitle: { fontSize: 13, fontWeight: 700, color: C.red, marginBottom: 4 },
  allergyWarningText: { fontSize: 12, color: C.muted, lineHeight: 1.5 },
  tabBar: {
  ...
  // Shared premium-lock panel (used by the jetlag meal-timing lock)
  nearbyLock: {
    display: "flex", flexDirection: "column", alignItems: "center",
